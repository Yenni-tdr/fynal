        ...
        <div>
            ...
            <div className='flex justify-end gap-4 mr-10'>
                <select className="select select-bordered w-full max-w-xs">
                    ...
                    <option onSelect={() => handleBouton(FILTRE_PRIX_CROISSANT_STRING)}>{FILTRE_PRIX_CROISSANT_STRING}</option>
                    <option onSelect={() => handleBouton(FILTRE_PRIX_DECROISSANT_STRING)}>{FILTRE_PRIX_DECROISSANT_STRING}</option>
                    <option onSelect={() => handleBouton(FILTRE_ALPHABETIQUE_CROISSANT_STRING)}>{FILTRE_ALPHABETIQUE_CROISSANT_STRING}</option>
                    <option onSelect={() => handleBouton(FILTRE_ALPHABETIQUE_DECROISSANT_STRING)}>{FILTRE_ALPHABETIQUE_DECROISSANT_STRING}</option>
                </select>
                <div className='flex flex-col gap-1'>
                    <span><input type="checkbox" id='stockCheckbox' className="checkbox" onClick={(eventCheckbox) => handleBouton(FILTRE_STOCK, eventCheckbox)}/> {FILTRE_STOCK}</span>
                    <span><input type='checkbox' id='reductionCheckbox' className="checkbox" onClick={(eventCheckbox) => handleBouton(FILTRE_REDUCTION, eventCheckbox)}/> {FILTRE_REDUCTION}</span>
                </div>
            </div>
