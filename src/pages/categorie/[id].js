...
async function newCartData(product, commande){
    ...
    const existItem = IDCOMMANDE === 0 ? undefined :  commande.PanierProduit.find((x)=>x.idProduit===product.idProduit)
    ...
    const response = await fetch('../api/panierAddButton', {
        method : 'POST',
        body: JSON.stringify({idProduit: product.idProduit, idCommande : IDCOMMANDE, quantite : quantity, exist : exist}),
    })
